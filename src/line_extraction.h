...
struct LineCovariances {
  ...
  LineCovariances()
      : start_point_cov(Eigen::Matrix2f::Identity()),
        end_point_cov(Eigen::Matrix2f::Identity()) {}

  LineCovariances(const Eigen::Matrix2f& start_point_cov,
                  const Eigen::Matrix2f& end_point_cov)
...
Eigen::Matrix2f GetSensorCovariance(const vector<double> ranges,
                                    const vector<double> angles);
vector<LineCovariances> GetLineEndpointCovariances(
    const vector<LineSegment> lines, const vector<Vector2f>& points);
