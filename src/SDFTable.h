...
class SDFTable {
 ...
 protected:
  ...
  inline double getPointWeight(int x, int y) const { return weights_(x, y); }

  inline double getPointDistance(int x, int y) const {
    return distances_(x, y);
  }

  inline void setPointWeight(int x, int y, double w) { weights_(x, y) = w; }

  inline void setPointDistance(int x, int y, double w) { distances_(x, y) = w; }
 ...
 public:
  ...
  SDFTable(const double range, const double resolution)
      : SDFTable(floor((range * 2.0) / resolution),
                 floor((range * 2.0) / resolution), resolution){};
  ...
  CImg<double> GetWeightDebugImage() const { return weights_; }

  CImg<double> GetDistanceDebugImage() const { return distances_; }
  ...
  void populateFromScan(sensor_msgs::LaserScan &laser_scan, bool truncate_ends,
                        ...
                        Eigen::Rotation2Df orientation);
  void normalizeWeights();
  void updateWithSDF(SDFTable &shortTermSDF);

  void filterCloud(const std::vector<Vector2f> &point_cloud,
                   std::vector<Vector2f> &filtered_point_cloud);
  void filterScan(sensor_msgs::LaserScan &laser_scan,
                  sensor_msgs::LaserScanPtr filtered_scan, bool truncate_ends,
                  Vector2f location, Eigen::Rotation2Df orientation);
};
