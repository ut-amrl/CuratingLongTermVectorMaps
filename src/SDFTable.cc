...
void SDFTable::updateWeightAndDistance(Vector2f point, double signed_dist) {
  ...
  double curr_d =
      ((signed_dist > Constants::delta)
           ? Constants::delta
           : (abs(signed_dist) <= Constants::delta)
                 ? signed_dist
                 : (signed_dist > -Constants::delta) ? -Constants::delta
                                                     : saved_distance);
  double curr_w = ((abs(signed_dist) < Constants::epsilon)
                       ? 1
                       : (abs(signed_dist) <= Constants::delta) ? exp(G) : 0);

  double new_w = (curr_w + saved_weight);
  if (new_w > 0) {
    double new_d = ((saved_weight * saved_distance) + (curr_w * curr_d)) /
                   (saved_weight + curr_w);
    setPointDistance(point, new_d);
    // printf("Distances: %f\t%f\t%f\t%f\t\t", signed_dist, saved_distance,
    // curr_d, new_d); printf("Weights: %f\t%f\t%f\t\n", saved_weight, curr_w,
    // new_w);
  }
...
void SDFTable::populateFromScan(sensor_msgs::LaserScan& laser_scan,
  ...
  for (size_t index = 0; index < laser_scan.ranges.size(); index++) {
    ...
    if (range >= laser_scan.range_min && range <= laser_scan.range_max &&
      ...
      for (float sub_range = 0; sub_range < range + Constants::delta;
           sub_range += resolution_) {
